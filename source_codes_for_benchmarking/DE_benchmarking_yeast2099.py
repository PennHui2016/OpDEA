...
def get_comparable_res(proteins, score, label, const, logFC,real_logFC, in_files):
    #fasta_name = in_files['database']
    #fasta, out = get_all_proteins(fasta_name)
    ...
    for con in uni_cons:
        ...
        #real_logFC_c.extend(real_logFC)
        #real_logFC_c.extend(get_true_logFC(label, contrast))
...
def run_all_group1(acq, tools, in_type, imputs, normals, in_files, dataset):
    ## the tools can select any type of paras
    #tools = ['limma', 'edgeR', 'siggenes', 'proDA', 'DEP', 'DEqMS', 'plgem', 'msqrob2', 'ProteoMM', 'Msstats', 'ProPCA', 'ANOVA', 'ttest', 'beta_binomial']
    for tool in tools:
        if in_type == 'maxquant':
            ...
            for imput2 in imputs:
                for normal1 in normals:
                    for normal2 in normals:
                        save_fold = in_files['save'] + dataset + '_' + acq + '/' + tool + '/'
                        # print('Rscript ' + R_fold + 'ProPCA' + '_DE.R' + ' ' + in_files['protein'] + ' ' + in_files[
                        #     'peptide']
                        #       + ' ' + in_files['design'] + ' ' + in_files[
                        #           'msstats'] + ' ' + in_type + ' ' + inten_ty + ' '
