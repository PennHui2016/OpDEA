...
save_folder = 'ensemble_metrics_top3_mean_DDA/'
...
def ensemble_DEA_threeinputs(dataset, platform, method, mode, in_files):
    ...
    if dataset == 'human_ecoli_DDA':
        if platform == 'fragpipe':
            ...
            wf2 = file_root + dataset + '/DEqMS/' + str(mode) + '/DEqMS_fragpipe_MaxLFQ_MinProb_.csv '
            wf3 = file_root + dataset + '/ProteoMM/' + str(mode) + '/ProteoMM_fragpipe_MaxLFQ_MinDet_max.csv '
        elif platform == 'maxquant':
            wf1 = file_root + dataset + '/plgem/' + str(mode) + '/plgem_maxquant__QRILC_div.mean.csv '  # plgem_maxquant__knn_sum
            wf2 = file_root + dataset + '/DEqMS/' + str(mode) + '/DEqMS_maxquant__MLE_center.median.csv '
            wf3 = file_root + dataset + '/ProteoMM/' + str(mode) + '/ProteoMM_maxquant_LFQ_MinDet_max.csv '
    else:
        if platform == 'fragpipe':
            wf1 = file_root + dataset + '/' + mode + '/plgem/plgem_fragpipe__QRILC_div.mean.csv ' #plgem_maxquant__knn_sum
            wf2 = file_root + dataset + '/' + mode + '/DEqMS/DEqMS_fragpipe_MaxLFQ_MinProb_.csv '
            wf3 = file_root + dataset + '/' + mode + '/ProteoMM/ProteoMM_fragpipe_MaxLFQ_MinDet_max.csv '
        elif platform == 'maxquant':
            wf1 = file_root + dataset + '/' + mode + '/plgem/plgem_maxquant__QRILC_div.mean.csv '  # plgem_maxquant__knn_sum
            wf2 = file_root + dataset + '/' + mode + '/DEqMS/DEqMS_maxquant__MLE_center.median.csv '
            wf3 = file_root + dataset + '/' + mode + '/ProteoMM/ProteoMM_maxquant_LFQ_MinDet_max.csv '

    wfs = [wf1 + wf2, wf1 + wf3, wf2 + wf3, wf1 + wf2 + wf3]
    names = ['sc_pro', 'sc_pep', 'pro_pep', 'sc_pro_pep']
    ks = [2,2,2,3]
    for wf in range(len(wfs)):
        ...
        if method == 'set':
            for operation in ['min', 'max', 'median']:
                save_fold = file_root + '/ensemble_res_mean1/' + dataset + '_' + str(mode) + '_' + platform + '_' + method + '_' + operation + '.csv'
                DEA_res_files = save_fold + ' ' + operation + ' ' + wfu
                print('Rscript ' + R_fold + 'ensemble_set.R ' + DEA_res_files)
                os.system('Rscript ' + R_fold + 'ensemble_set.R ' + DEA_res_files)
                if dataset == 'human_ecoli_DDA':
                    cal_mse_auc1('ensemble_' + nameu + '_' + method + '_' + operation, 'DDA', platform, '_', '_', '_', save_fold, dataset,
                                0, in_files, mode, k)
                else:
                    cal_mse_auc('ensemble_' + nameu + '_' + method + '_' + operation, 'DDA', platform, '_', '_', '_', save_fold, dataset, 0, in_files, mode, k)
        elif method == 'fisher':
            save_fold = file_root + '/ensemble_res_mean1/' + dataset + '_' + str(mode) + '_' + platform + '_' + method + '_' + '.csv'
            DEA_res_files = save_fold + ' ' + wfu
            print('Rscript ' + R_fold + 'ensemble_fisher.R ' + DEA_res_files)
            os.system('Rscript ' + R_fold + 'ensemble_fisher.R ' + DEA_res_files)
            if dataset == 'human_ecoli_DDA':
                cal_mse_auc1('ensemble_' + nameu + '_' + method, 'DDA', platform, '_', '_', '_', save_fold, dataset, 0,
                            in_files, mode, k)
            else:
                cal_mse_auc('ensemble_' + nameu + '_' + method, 'DDA', platform, '_', '_', '_', save_fold, dataset, 0,
                        in_files, mode, k)
...
def ensemble_DEA_topKwf(dataset, platform, method, mode, in_files, wfs, acq, k):
    ...
    if method == 'set':
        for operation in ['min', 'max', 'median']:
            save_fold = file_root + '/ensemble_res_DIA/' + dataset + '_' + str(mode) + '_' + platform + '_' + method + '_' + operation + '.csv'
        ...
        save_fold = file_root + '/ensemble_res_DIA/' + dataset + '_' + str(mode) + '_' + platform + '_' + method + '_' + '.csv'
...
if __name__ == '__main__':
    ...
    for in_type in ['fragpipe', 'maxquant']:
        ...
        for method in ['set', 'fisher', 'hurdle']:
            ensemble_DEA_threeinputs(dataset, in_type, method, '', in_files)
    ...
    for in_type in ['fragpipe', 'maxquant']:
        ...
        for comb_idx in comb_idxs:
            ...
            for method in ['set', 'fisher', 'hurdle']:
                ensemble_DEA_threeinputs(dataset, in_type, method, str(comb_idx), in_files)