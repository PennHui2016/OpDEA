...
def get_true_logFC(label, contrast):
    ...
    true_logFC[np.where((label == 1) & (contrast == 'conditionB-conditionA'))[0], 0] = np.log2(0.25 / 0.1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionC-conditionA'))[0], 0] = np.log2(1 / 0.1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionD-conditionA'))[0], 0] = np.log2(2.5 / 0.1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionE-conditionA'))[0], 0] = np.log2(5 / 0.1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionF-conditionA'))[0], 0] = np.log2(10 / 0.1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionG-conditionA'))[0], 0] = np.log2(25 / 0.1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionH-conditionA'))[0], 0] = np.log2(50 / 0.1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionC-conditionB'))[0], 0] = np.log2(1 / 0.25)
    true_logFC[np.where((label == 1) & (contrast == 'conditionD-conditionB'))[0], 0] = np.log2(2.5 / 0.25)
    true_logFC[np.where((label == 1) & (contrast == 'conditionE-conditionB'))[0], 0] = np.log2(5 / 0.25)
    true_logFC[np.where((label == 1) & (contrast == 'conditionF-conditionB'))[0], 0] = np.log2(10 / 0.25)
    true_logFC[np.where((label == 1) & (contrast == 'conditionG-conditionB'))[0], 0] = np.log2(25 / 0.25)
    true_logFC[np.where((label == 1) & (contrast == 'conditionH-conditionB'))[0], 0] = np.log2(50 / 0.25)
    true_logFC[np.where((label == 1) & (contrast == 'conditionD-conditionC'))[0], 0] = np.log2(2.5 / 1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionE-conditionC'))[0], 0] = np.log2(5 / 1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionF-conditionC'))[0], 0] = np.log2(10 / 1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionG-conditionC'))[0], 0] = np.log2(25 / 1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionH-conditionC'))[0], 0] = np.log2(50 / 1)
    true_logFC[np.where((label == 1) & (contrast == 'conditionE-conditionD'))[0], 0] = np.log2(5 / 2.5)
    true_logFC[np.where((label == 1) & (contrast == 'conditionF-conditionD'))[0], 0] = np.log2(10 / 2.5)
    true_logFC[np.where((label == 1) & (contrast == 'conditionG-conditionD'))[0], 0] = np.log2(25 / 2.5)
    true_logFC[np.where((label == 1) & (contrast == 'conditionH-conditionD'))[0], 0] = np.log2(50 / 2.5)
    true_logFC[np.where((label == 1) & (contrast == 'conditionF-conditionE'))[0], 0] = np.log2(10 / 5)
    true_logFC[np.where((label == 1) & (contrast == 'conditionG-conditionE'))[0], 0] = np.log2(25 / 5)
    true_logFC[np.where((label == 1) & (contrast == 'conditionH-conditionE'))[0], 0] = np.log2(50 / 5)
    true_logFC[np.where((label == 1) & (contrast == 'conditionG-conditionF'))[0], 0] = np.log2(25 / 10)
    true_logFC[np.where((label == 1) & (contrast == 'conditionH-conditionF'))[0], 0] = np.log2(50 / 10)
    true_logFC[np.where((label == 1) & (contrast == 'conditionH-conditionG'))[0], 0] = np.log2(50 / 25)
    return true_logFC
...
def cal_mse_auc(tool, acq, mode, in_type, inten_ty, imput, normal, res_file, dataset, run_time, in_files):
    if(os.path.exists(res_file)):
        ...
        protein_c, score_c, label_c, const_c, logFC_c, real_logFC_c = get_comparable_res(proteins, score, label, const,
                                                                                         logFC, real_logFC, in_files)

        p = 0.01
        aucs, paucs, sprs, rmses, lens, Cls_met_1_001, Cls_met_1_005 = cal_auc(score, label, const, p, logFC, real_logFC)
        ...
        aucs, paucs, sprs, rmses, lens, Cls_met_1_001, Cls_met_1_005 = cal_auc(score_c, label_c, const_c, p, logFC_c,
                                                                               real_logFC_c)
...
def run_Rscript(acq, tool, in_type, inten_ty, imput, normal, in_files, dataset, mode):
    '''
    :param tool: DE tool
    ...
    :return: DE results
    '''
    save_fold = in_files['save'] + dataset + '_' + acq + '/' + mode + '/' + tool + '/'
    t1 = time.time()
    ...
    cal_mse_auc(tool, acq, mode, in_type, inten_ty, imput, normal, res_file_name, dataset, run_time, in_files)


def run_all_group1(acq, tools, in_type, imputs, normals, in_files, dataset, mode):
        ...
        for inten_ty in inten_types:
            for imput in imputs:
                for normal in normals:
                    run_Rscript(acq, tool, in_type, inten_ty, imput, normal, in_files, dataset, mode)

def run_all_group2(acq, tools, in_type, imputs, normals, in_files, dataset, mode):
    for tool in tools:
        for imput in imputs:
            for normal in normals:
                run_Rscript(acq, tool, in_type, 'basic', imput, normal, in_files, dataset, mode)

def run_Mstats(acq, in_files, in_type, dataset, mode):
    in_files.update({'design':in_files['design_stat']})
    for normal in ['equalizeMedians', 'quantile', 'globalStandards', 'FALSE']:
        run_Rscript(acq, 'MSstats', in_type, 'basic', 'blank', normal, in_files, dataset, mode)

def run_ProPCA(acq, in_files, in_type, dataset, mode):
    tool = 'ProPCA'
            ...
            for imput2 in imputs:
                for normal1 in normals:
                    for normal2 in normals:
                        save_fold = in_files['save'] + dataset + '_' + acq + '/' + tool + '/' + mode + '/'
...
def run_all(acq, in_files, dataset, in_type, flag, mode):

    tool_group1 = ['limma', 'siggenes']
    tool_group4 = ['DEP','ANOVA', 'ttest']
    ...
    if flag == 1:
        run_all_group1(acq, tool_group1, in_type, imputs1, normals, in_files, dataset, mode)
    elif flag == 2:
        run_all_group2(acq, tool_group2, in_type, imputs1, normals, in_files, dataset, mode)
    elif flag == 3:
        run_all_group1(acq, tool_group3, in_type, imputs1, normals, in_files, dataset, mode)

    elif flag == 4:
        run_Mstats(acq, in_files, in_type, dataset, mode)
    elif flag == 5:

        run_all_group1(acq, tool_group7, in_type, imputs1, normals, in_files, dataset, mode)
    elif flag == 6:
        run_all_group1(acq, tool_group4, in_type, imputs1, normals, in_files, dataset, mode)
    elif flag == 7:
        run_all_group1(acq, tool_group5, in_type, imputs1, normals, in_files, dataset, mode)
    elif flag == 8:
        run_all_group1(acq, tool_group6, in_type, imputs1, normals, in_files, dataset, mode)
...
if __name__ == '__main__':
    ...
    if acq == 'DIA':
        if in_type == 'maxquant':
            ...
            in_files = {'protein': '/home/hui/Documents/VM_share/DIA_data/ecoli/DIANN/' + mode + '/report.pg_matrix.tsv',
                        ...
                        'labels': '/home/hui/Documents/VM_share/DIA_data/A549_K562/CL_labeling.txt',
                        'database': '/home/hui/Documents/VM_share/DIA_data/REF_EColi_K12_UPS1_combined.fasta',
                        'pro_path': '/home/hui/Documents/VM_share/DIA_data/ecoli/DIANN/' + mode + '/'
                        }
    elif acq == 'DDA':
        if in_type == 'maxquant':
    ...
    run_all(acq, in_files, dataset, in_type, flag, mode)
