...
for (i in 1:length(consts$conts)) {
  ...
  row.names(condition)<-designs[c(grep(g1, designs$condition), grep(g2, designs$condition)),]$sample.name
  data_test = ExpressionSet(as.matrix(counts.filter), phenoData = AnnotatedDataFrame(data=condition))
  set.seed(123)
  LPSdegList <- run.plgem(esdata=data_test)

  res.plgem<-as.data.frame(row.names(counts.filter))
  res.plgem$STN<-LPSdegList$PLGEM.STN
  res.plgem$logFC<-log2(LPSdegList$PLGEM.STN)
  #res.plgem$logFC<-log2(colMeans(counts.filter[,c(grep(g2,condition$conditionName))])/colMeans(counts.filter[,c(grep(g1,condition$conditionName))]))
...
colnames(res_all)[1:4]<-c('protein', 'STN', 'logFC', 'pvalue', 'adj.pvalue')

res_all<-res_all[order(res_all$pvalue),]
write.table(res_all, paste0(save_fold, 'plgem_', in_type, '_', inten_type, '_', imput, '_', normal, '.csv'), sep = ',', row.names = TRUE, col.names = TRUE)
