...
procedure_compare<-function(data_fold, procedure, st, en, platform, acq, wb){ #ranks,
  ...
  #return(list(fig=p3, diffs=diffs, ranks=rank_fg_matrix_TMT, dat=out_data))
}

library(openxlsx)
wb <- createWorkbook()
procedure_compare(data_fold, 'Matrix', 7, 28, 'FragPipe','DDA',wb)
procedure_compare(data_fold, 'DEA', 7, 28, 'FragPipe','DDA',wb)
procedure_compare(data_fold, 'DEA_count', 7, 28, 'FragPipe','DDA',wb)
procedure_compare(data_fold, 'Imput', 7, 28, 'FragPipe','DDA',wb)
procedure_compare(data_fold, 'normalization', 7, 28, 'FragPipe','DDA',wb)

wb <- createWorkbook()
procedure_compare(data_fold, 'Matrix', 7, 28, 'Maxquant','DDA',wb)
procedure_compare(data_fold, 'DEA', 7, 28, 'Maxquant','DDA',wb)
procedure_compare(data_fold, 'DEA_count', 7, 28, 'Maxquant','DDA',wb)
procedure_compare(data_fold, 'Imput', 7, 28, 'Maxquant','DDA',wb)
procedure_compare(data_fold, 'normalization', 7, 28, 'Maxquant','DDA',wb)

wb <- createWorkbook()
procedure_compare(data_fold, 'Matrix', 7, 21, 'FragPipe','TMT',wb)
procedure_compare(data_fold, 'DEA', 7, 21, 'FragPipe','TMT',wb)
procedure_compare(data_fold, 'Imput', 7, 21, 'FragPipe','TMT',wb)
procedure_compare(data_fold, 'normalization', 7, 21, 'FragPipe','TMT',wb)

wb <- createWorkbook()
procedure_compare(data_fold, 'DEA', 7, 21, 'Maxquant','TMT',wb)
procedure_compare(data_fold, 'Imput', 7, 21, 'Maxquant','TMT',wb)
procedure_compare(data_fold, 'normalization', 7, 21, 'Maxquant','TMT',wb)

wb <- createWorkbook()
procedure_compare(data_fold, 'Matrix', 7, 23, 'DIANN','DIA',wb)
procedure_compare(data_fold, 'DEA', 7, 23, 'DIANN','DIA',wb)
procedure_compare(data_fold, 'Imput', 7, 23, 'DIANN','DIA',wb)
procedure_compare(data_fold, 'normalization',7, 23, 'DIANN','DIA',wb)

wb <- createWorkbook()
procedure_compare(data_fold, 'Matrix', 7, 23, 'spt','DIA',wb)
procedure_compare(data_fold, 'DEA', 7, 23, 'spt','DIA',wb)
procedure_compare(data_fold, 'Imput', 7, 23, 'spt','DIA',wb)
procedure_compare(data_fold, 'normalization',7, 23, 'spt','DIA',wb)
...
col=c("#a6cee3","#33a02c","#fb9a99","#fdbf6f","#1f78b4","#b2df8a")#c("#1f78b4","#33a02c","#fdbf6f","#e31a1c")
p1<-ggplot(cb_diff,aes(x=contrast, y=diff, color=setting))+
  geom_boxplot(outlier.shape = NA,varwidth = FALSE)+
  #geom_hline(aes(yintercept = 0, colour = "red"),show.legend = FALSE)+
  stat_summary(fun=mean,
               geom="point",
               shape=20,
               aes(group=setting),
               position=position_dodge(.75), size=2, color="red", fill="red")+
  geom_hline(aes(yintercept = 0), linetype = 2,colour='red',show.legend = FALSE)+
  guides(color=guide_legend(nrow=1, byrow=TRUE))+ylim(-0.25,0.5)+scale_color_manual(values = col)+#
  theme_pubr()+
...
for (i in 1:length(procedures)) {
  ...
  if(procedure!='Matrix'){
    idx<-match(heatmap_rank$Method, DEA_run_times$Method)
    heatmap_rank$time<-DEA_run_times$time[idx]
...
for (i in 1:length(procedures)) {
  ...
  addWorksheet(wb,sheet)
  writeData(wb, sheet, ranks_procedure, startRow = 1, startCol = 1, colNames = TRUE, rowNames = TRUE)
  saveWorkbook(wb, file = paste0(save_fold, 'Heatmap.xlsx'), overwrite = TRUE)
  sheet_label=paste0(procedures[i], '_lab')
  addWorksheet(wb,sheet_label)
  writeData(wb, sheet_label, ranks_procedure_label, startRow = 1, startCol = 1, colNames = TRUE, rowNames = TRUE)
  saveWorkbook(wb, file = paste0(save_fold, 'Heatmap.xlsx'), overwrite = TRUE)
...
DEA_run_times<-read_excel(paste0(data_fold,'/times.xlsx'), sheet = 'DEA', col_names = F)
colnames(DEA_run_times)<-c('Method','time')
DEA_run_times<-DEA_run_times[which(DEA_run_times$Method!='MSstats'),]
Imput_run_times<-read_excel(paste0(data_fold,'/times.xlsx'), sheet = 'Imput', col_names = F)
colnames(Imput_run_times)<-c('Method','time')
norm_run_times<-read_excel(paste0(data_fold,'/times.xlsx'), sheet = 'normalization', col_names = F)
colnames(norm_run_times)<-c('Method','time')
...
##Figure 4B
p3<-Heatmap(as.matrix(heatmap_dt), name = "rank", km = 0, cluster_rows = FALSE,
...
p31<-Heatmap(as.matrix(ha), name = "time(s)", km = 0, cluster_rows = FALSE,
             cluster_columns = FALSE,#row_order =as.matrix(data_fragpipe$workflow), #ranking_frag,
...
write.table(ha, paste0(save_fold, 'Figure4B_2.csv'), col.names = T, row.names = T)
...
heatmap_dt<-heatmap_dt[,2:(length(heatmap_dt[1,])-1)]
heatmap_lab<-heatmap_lab[,2:(length(heatmap_lab[1,])-1)]
#heatmap(as.matrix(heatmap_dt), Colv = NA, Rowv = NA, scale="column")

## Figure 4C
p2<-Heatmap(as.matrix(heatmap_dt), name = "rank", km = 0, cluster_rows = FALSE,
            cluster_columns = FALSE,#row_order =as.matrix(data_fragpipe$workflow), #ranking_frag,
            show_row_names = TRUE, row_names_side = 'right', show_column_names = TRUE,
...
p21<-Heatmap(as.matrix(ha), name = "time(s)", km = 0, cluster_rows = FALSE,
             cluster_columns = FALSE,#row_order =as.matrix(data_fragpipe$workflow), #ranking_frag,
             show_row_names = FALSE, show_column_names = TRUE,
...
p1<-Heatmap(as.matrix(heatmap_dt), name = "rank", km = 0, cluster_rows = FALSE,
            cluster_columns = FALSE,#row_order =as.matrix(data_fragpipe$workflow), #ranking_frag,
            show_row_names = TRUE, row_names_side = 'right', show_column_names = TRUE,
...
p11<-Heatmap(as.matrix(ha), name = "time(s)", km = 0, cluster_rows = FALSE,
             cluster_columns = FALSE,#row_order =as.matrix(data_fragpipe$workflow), #ranking_frag,
...
write.table(heatmap_dt, paste0(save_fold, 'Figure4C_3.csv'), col.names = T, row.names = T)
write.table(ha, paste0(save_fold, 'Figure4C_4.csv'), col.names = T, row.names = T)
...
p4<-Heatmap(as.matrix(heatmap_dt), name = "rank", km = 0, cluster_rows = FALSE,
            cluster_columns = FALSE,#row_order =as.matrix(data_fragpipe$workflow), #ranking_frag,
            show_row_names = TRUE, row_names_side = 'right', show_column_names = TRUE,
            column_names_side = 'bottom', heatmap_width=unit(130, "mm"),heatmap_height=unit(80, "mm"), row_names_centered = TRUE,
            column_names_rot=-20, column_names_centered = TRUE,
...
p41<-Heatmap(as.matrix(ha), name = "time(s)", km = 0, cluster_rows = FALSE,
             cluster_columns = FALSE,#row_order =as.matrix(data_fragpipe$workflow), #ranking_frag,
             show_row_names = FALSE, show_column_names = TRUE,
...
#row.names(heatmap_dt)[length(heatmap_dt$Method)]<-'reporter intensity'
...
#heatmap_lab$Method[length(heatmap_dt$Method)]<-'reporter intensity'
...
p5<-Heatmap(as.matrix(heatmap_dt), name = "rank", km = 0, cluster_rows = FALSE,
            cluster_columns = FALSE,#row_order =as.matrix(data_fragpipe$workflow), #ranking_frag,
            show_row_names = TRUE, row_names_side = 'right', show_column_names = TRUE,
            column_names_side = 'bottom', heatmap_width=unit(130, "mm"),heatmap_height=unit(80, "mm"), row_names_centered = TRUE,
            column_names_rot=-20, column_names_centered = TRUE,
