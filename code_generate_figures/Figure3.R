...
for (i in 1:length(platforms)){
  cls_cv_res<-read.table(paste0(save_fold, platforms[i], '_', acqs[i], '_', 'cv_cbt_cls_res_folds_str'),sep = ',')
...
sheet='10-fold_cv_res'
addWorksheet(wb,sheet)
writeData(wb, sheet, cls_res_fig, startRow = 1, startCol = 1, colNames = TRUE, rowNames = TRUE)
saveWorkbook(wb, file = paste0(save_fold,'10-fold_cv_res.xlsx'), overwrite = TRUE)
...
write.table(cls_res_fig, paste0(save_fold, 'Figure3A.csv'), col.names = T, row.names = F)
...
setting<-c('MQ_TMT','FG_TMT','FG_DDA','MQ_DDA','DIANN_DIA','spt_DIA')
importances<-vector()
#Maxquant_TMT_fea_importance.csv
for (i in 1:length(platforms)){
  cls_impt<-read.table(paste0(save_fold, platforms[i], '_', acqs[i], '_', 'fea_importance.csv'),sep = ',', header = T)
...
addWorksheet(wb,sheet)
writeData(wb, sheet, importances_fig, startRow = 1, startCol = 1, colNames = TRUE, rowNames = TRUE)
saveWorkbook(wb, file = paste0(save_fold,'10-fold_cv_res.xlsx'), overwrite = TRUE)
...
wb <- createWorkbook()
for(i in 1:length(platforms)){
  hfp<-read.table(paste0(save_fold, platforms[i],'_',acqs[i],'FP_freitem_H.csv'),sep=',', header=F)
  ...
  lfp<-read.table(paste0(save_fold, platforms[i],'_',acqs[i],'FP_freitem_L.csv'),sep=',', header=F)
  ...
  addWorksheet(wb,sheet)
  writeData(wb, sheet, hfp, startRow = 1, startCol = 1, colNames = TRUE, rowNames = TRUE)
  sheet=paste0(setting[i], '_L')
  addWorksheet(wb,sheet)
  writeData(wb, sheet, lfp, startRow = 1, startCol = 1, colNames = TRUE, rowNames = TRUE)
  saveWorkbook(wb, file = paste0(save_fold, 'FPs_all.xlsx'), overwrite = TRUE)
}
...
p71<-p7+
  ...
  geom_vline(xintercept = c(4.5,8.5,12.5,15.5,19.5),
             linetype='dashed')
mytheme = theme_classic() + theme(axis.text.x = element_text(size = 14),axis.text.y = element_text(size = 14))+
  theme(axis.title.y= element_text(size=14))+theme(axis.title.x = element_text(size = 14))+
