...
acq='DDA'
setting = 'FG_DDA'
ranking_all<-read_excel(paste0(data_fold, 'workflows.xlsx'), sheet = setting)
...
ranking_all$label[as.integer(length(ranking_all$workflow)*0.05):as.integer(length(ranking_all$workflow)*0.25)] = '5-25%'
ranking_all$label[as.integer(length(ranking_all$workflow)*0.25):as.integer(length(ranking_all$workflow)*0.5)] = '25-50%'
ranking_all$label[as.integer(length(ranking_all$workflow)*0.5):as.integer(length(ranking_all$workflow))] = '50-100%'
...
## Figure supp. Fig. 3 in Supplementary results.
...
p11 = ggplot(dat_l, aes(x=metrics, y=performance, fill=label)) +
  geom_violin(position = position_dodge(width = 0.75), scale = 'width')+
  scale_fill_manual(values = col)+
  labs(x="", y="performance", color=factor)+
  scale_x_discrete(limits=c("pAUC(0.01)","pAUC(0.05)","pAUC(0.1)",'nMCC', 'G-mean'))
...
spr_test<-function(platform, acq){
  ...
  nMCC<-read_excel(paste0(data_fold, 'metrics_',platform, '_',acq, '.xlsx'), sheet='nMCC')
  Gmean<-read_excel(paste0(data_fold, 'metrics_',platform, '_',acq, '.xlsx'), sheet='G-mean')
...
lopocv_fg_dda<-spr_test('FG', 'DDA')
lopocv_mq_dda<-spr_test('MQ', 'DDA')
lopocv_fg_tmt<-spr_test('FG', 'TMT')
lopocv_mq_tmt<-spr_test('MQ', 'TMT')
...
p2<-ggplot(scatter_dat, aes(new, bechmarking, color = method)) +
  ...
  stat_cor(method = "spearman",aes(color = method),label.x = c(5000,5000), label.y = c(800,2000)) +
...
## Figure supp. Fig. 1 in Supplementary results
...
p3 = ggplot(sprs_dat, aes(x=setting, y=spearman, fill=method)) +
  geom_boxplot(position = position_dodge(width = 0.3), width=0.3)+
...
fg_avg_anova_DDA<-ANOVA_lm('FG', 'DDA', 7, 28, wb)
mq_avg_anova_DDA<-ANOVA_lm('MQ', 'DDA', 7, 28, wb)
diann_avg_anova_DIA<-ANOVA_lm('DIANN', 'DIA', 7, 23, wb)
spt_avg_anova_DIA<-ANOVA_lm('spt', 'DIA', 7, 23, wb)

rank_fg_DDA<-read_excel(paste0(data_fold,'workflows', '.xlsx'), sheet = 'FG_DDA')
rank_mq_DDA<-read_excel(paste0(data_fold,'workflows', '.xlsx'), sheet = 'MQ_DDA')
rank_fg_TMT<-read_excel(paste0(data_fold,'workflows', '.xlsx'), sheet = 'FG_TMT')
rank_mq_TMT<-read_excel(paste0(data_fold,'workflows', '.xlsx'), sheet = 'MQ_TMT')
rank_diann_DIA<-read_excel(paste0(data_fold,'workflows', '.xlsx'), sheet = 'DIANN_DIA')
rank_spt_DIA<-read_excel(paste0(data_fold,'workflows', '.xlsx'), sheet = 'spt_DIA')
...
library(ggrepel)
p4<-ggplot(melt_anova_ps_dat, aes(x=rank, y=log10p,color=stable, shape=setting)) +
  geom_point(alpha=1, size=3) +
  theme_bw(base_size = 12) +
  scale_shape_manual(values=c(1, 3,4, 8, 13, 24))+
  xlab("ranking of workflows") +
  ylab("-Log10(pvalue) of Kruskal-Wallis test") +
  scale_colour_manual(values = c("purple","#e31a1c",'gray')) +
...
sheet='dataset'
addWorksheet(wb,sheet)
writeData(wb, sheet, melt_anova_ps_dat, startRow = 1, startCol = 1, colNames = TRUE, rowNames = TRUE)
saveWorkbook(wb, file = paste0(save_fold,'top30_Kruskal-Wallis.xlsx'), overwrite = TRUE)
...
rank_frag_DDA<-read_excel(paste0(data_fold,'workflows.xlsx'), sheet = 'FG_DDA')
rank_mq_DDA<-read_excel(paste0(data_fold,'workflows.xlsx'), sheet = 'MQ_DDA')
rank_frag_TMT<-read_excel(paste0(data_fold,'workflows.xlsx'), sheet = 'FG_TMT')
rank_mq_TMT<-read_excel(paste0(data_fold,'workflows.xlsx'), sheet = 'MQ_TMT')
rank_diann_DIA<-read_excel(paste0(data_fold,'workflows.xlsx'), sheet = 'DIANN_DIA')
rank_spt_DIA<-read_excel(paste0(data_fold,'workflows.xlsx'), sheet = 'spt_DIA')
...
plot_topwf<-function(i, cols){
  rank_res<-read_excel(paste0(data_fold,'workflows.xlsx'), sheet = setting[i])
  rank_res<-rank_res[order(rank_res$avg_rank_mean),]
  rank_res$exact_rank<-c(1:length(rank_res$workflow))
  rank_res<-rank_res[which(rank_res$DEA_tool!='MSstats'),]
  uni_matrix<-unique(rank_res$expression_matrix)
  top_ranks<-data.frame()
  for (j in 1:length(uni_matrix)) {
    idx_mat<-which(rank_res$expression_matrix==uni_matrix[j])
    ...
    top_rank$Matrix<-gsub('intensity','reporter',top_rank$expression_matrix)
    top_rank$Imput<-gsub('blank','none',top_rank$imputation)
    top_rank$normalization<-gsub('blank','none',top_rank$normalization)
    top_rank$DEA<-top_rank$DEA_tool
  ...
  p8
  return(list(p=p8, top=top_ranks))
}
...
# supp. Fig. 2
p81<-plot_topwf(1, col[7]) # MQ_TMT
p81$p
p82<-plot_topwf(2, col[8:10]) # FG_TMT
p82$p
...
write.table(p83$top, paste0(save_fold, 'Figure2E_1.csv'), col.names = T, row.names = F)
write.table(p84$top, paste0(save_fold, 'Figure2E_2.csv'), col.names = T, row.names = F)
write.table(p85$top, paste0(save_fold, 'Figure2E_3.csv'), col.names = T, row.names = F)
write.table(p86$top, paste0(save_fold, 'Figure2E_4.csv'), col.names = T, row.names = F)
