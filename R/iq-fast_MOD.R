...
process_long_format <- function(input_filename,
    ...
    iq_dat <- iq::fast_read(input_filename,
    ...
    if (!is.null(iq_dat)) {
        ...
        if (method=='maxlfq'){
          message("start maxlfq!")
        res <- iq::fast_MaxLFQ(iq_norm_data,
                           row_names = iq_dat$protein[, 1],
                           col_names = iq_dat$sample)
        }else if(method!='maxlfq'){
          message(paste0("start ", method,"!"))
          iq_norm_data0<-iq_norm_data
          iq_norm_data0$protein_list=iq_dat$protein[iq_norm_data0$protein_list]
          iq_norm_data0$sample_list=iq_dat$sample[iq_norm_data0$sample_list]
          iq_norm_data_df<-as.data.frame(iq_norm_data0)
          protein_list <- create_protein_list(iq_norm_data_df)
          if (method!='topN'){
            res <- create_protein_table(protein_list, method = method)
          }else if(method=='topN'){
            #N = as.integer((gsub('top', '', method))
            res <- create_protein_table(protein_list, method = method, N = N)
          }
          # write.table(cbind(Protein = rownames(res$estimate),
          #                   res$estimate,
