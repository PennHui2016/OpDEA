...
def tprfpr(score, label, logFC, logfc_thre, score_thre):
    y_true = label
    y_pred = (score_thre <= score) & (abs(logFC) >= logfc_thre)
    #tn, fp, fn, tp = confusion_matrix(y_true, y_pred).ravel()
...
def partialAuc(fpr, tpr, max_fpr):
    if max_fpr is None or max_fpr == 1:
        return metrics.auc(fpr, tpr)
    elif max_fpr>=max(fpr):
        return metrics.auc(fpr, tpr)
...
def cal_auc(score, score_lfq, label, const, p, logFC, rea_logFC):
    ...
    if len(np.unique(label)) == 1:
        ...
        fpr0, tpr0, thre = roc_curve(label, score)
        tpr_prs = [tprfpr(score, label, logFC, np.log2(1.5), score_thre) for score_thre in thre]
        #tpr_prs = [tprfpr(score, label, logFC, 0, score_thre) for score_thre in thre]
        tpr_prs = np.array(tpr_prs).squeeze()

        auc = partialAuc(tpr_prs[:, 1], tpr_prs[:, 0], max_fpr=1)
        pauc1 = partialAuc(tpr_prs[:, 1], tpr_prs[:, 0], max_fpr=0.01)
        pauc2 = partialAuc(tpr_prs[:, 1], tpr_prs[:, 0], max_fpr=0.05)
        pauc3 = partialAuc(tpr_prs[:, 1], tpr_prs[:, 0], max_fpr=0.1)
    ...
    for con in uni_cons:
        ...
        if len(np.unique(label[idx])) == 1:
            ...
            fpr0, tpr0, thre = roc_curve(label[idx], score[idx])
            tpr_prs = [tprfpr(score[idx], label[idx], logFC[idx], np.log2(1.5), score_thre) for score_thre in thre]
            tpr_prs = np.array(tpr_prs).squeeze()
...
def run_Rscript(acq, tool, in_type, inten_ty, imput, normal, in_files, dataset, true_organism, DE_organism):
    ...
    #in_files['save_folder'] = in_files['save_folder'] + 
...
def run_all(acq, in_files, dataset, platform, flag, true_organism, DE_organism):
    ...
    normals = ['vsn','blank', "sum", "max", "center.mean", "center.median", "div.mean", "div.median", "quantiles",
