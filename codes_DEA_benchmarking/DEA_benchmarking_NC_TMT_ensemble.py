...
def run_Rscript(acq, tool, in_type, inten_ty, imput, normal, in_files, dataset, true_organism, DE_organism):
    ...
    if (inten_ty == 'abd') | (inten_ty == 'ratio'):
        logT = 'F'
    else:
        logT = 'T'
    ...
    if os.path.exists(res_file_name):
        ...
        t1 = time.time()
        os.system(
            in_files['Rscript_path'] + 'Rscript ' + R_fold + tool + '_DE.R' + ' ' + in_files['maxtrix_folder'] + ' ' +
            in_files['mstats_file'] +
            ' ' + in_files['evidence_file'] + ' ' + in_files['msstats_design_file'] + ' ' +
            in_files['main_output_protein_file'] + ' ' + in_files['main_output_peptide_file'] + ' ' +
            in_type + ' ' + inten_ty + ' ' +
...
def run_all_group1(acq, tools, platform, imputs, normals, in_files, dataset, true_organism,
    ...
    if platform == 'FragPipe':
        intens_ty = ['abd']#, 'ratio', 'phi']
    elif platform == 'Maxquant':
        intens_ty = ['intensity']
    for tool in tools:
        inten_types = intens_ty #['top0','top3', 'LFQ', 'dlfq']  # ['LFQ']#, 'dlfq']#
...
def run_all(acq, in_files, dataset, platform, flag, true_organism, DE_organism):
    # For TMT, only intensity based quantification
    tool_group1 = ['limma', 'ttest']#, 'siggenes'
...
def ensemble_top_multi_view(platform, acq, method, res_fold, save_fold, DEA_res_fold, dataset, in_files, true_organism, DE_organism,):
    #res_fold D:\data\benchmark\benchmark_res\ensemble
    # ranking_all = pd.read_csv(res_fold + acq + '_' + platform + '_ranks_all.csv', sep=',', header=0)
    ranking_all = pd.read_excel(res_fold + 'ranks_all_' + platform + '_' + acq + '.xlsx', sheet_name='ranking_all', header=0)
    # drop MSstats
    ranking_all = ranking_all[ranking_all['DEA']!='MSstats']
    #ranking_median = ranking_all['avg_rank_median'].values
    ranking_matrix = ranking_all['Matrix'].values
    uni_matrix = np.unique(ranking_matrix)
    from itertools import combinations
    cbns = list(combinations(uni_matrix, 2))
    if len(uni_matrix)>2:
        for n in range(3, len(uni_matrix)+1):
            cbns += list(combinations(uni_matrix, n))
    for cbn in cbns:
        top_wfs = []
        res_files = []
        wfs = ''
        cbn_str = ''
        for mtx in cbn:
            df_mtx = ranking_all[ranking_all['Matrix']==mtx]
            top_wf_idx = np.where((df_mtx['avg_rank_mean'].values==min(df_mtx['avg_rank_mean'].values)))[0][0]
            top_wfs.append(top_wf_idx)
            dea = df_mtx['DEA'].values[top_wf_idx]
            imput = df_mtx['Imput'].values[top_wf_idx]
            norm = df_mtx['normalization'].values[top_wf_idx]
            if imput == 'blank':
                imput = ''
            if norm == 'blank':
                norm = ''
            res_file = DEA_res_fold + dataset + '_' + dea + '_' + platform + '_' + mtx + '_' + imput + '_' + norm + '.csv'
        ...
        print_lab = 'T'
        true_logfc = in_files['true_lgfc']
        conss = in_files['contrasts']
        if method == 'set':
            for operation in ['min', 'max', 'median']:
                save_res_fold = save_fold + dataset + '/' + dataset + '_' + platform + '_' + method + '_' + operation + '_' + cbn_str.replace('|', '-') + '.csv'
                if not os.path.exists(save_fold + dataset + '/'):
                    os.makedirs(save_fold + dataset + '/')
                DEA_res_files = save_res_fold + ' ' + operation + ' ' + print_lab + ' ' + true_organism + ' ' + DE_organism + ' ' + true_logfc + ' ' + conss + ' ' + wfs
                print(in_files['Rscript_path'] + 'Rscript ' + R_fold + 'ensemble_set.R ' + DEA_res_files)
                os.system(in_files['Rscript_path'] + 'Rscript ' + R_fold + 'ensemble_set.R ' + DEA_res_files)

                cal_mse_auc('ensemble_' + method + '_' + operation, acq, platform, method, operation, len(cbn), save_res_fold,
                                dataset, cbn_str + '||' + wfs, in_files)
        elif method == 'fisher':
            save_res_fold = save_fold + dataset + '/' + dataset + '_' + platform + '_' + method + '_' + cbn_str.replace('|', '-') + '.csv'
            if not os.path.exists(save_fold + dataset + '/'):
                os.makedirs(save_fold + dataset + '/')
            DEA_res_files = save_res_fold + ' ' + print_lab + ' ' + true_organism + ' ' + DE_organism + ' ' + true_logfc + ' ' + conss + ' ' + wfs
            print(in_files['Rscript_path'] + 'Rscript ' + R_fold + 'ensemble_fisher.R ' + DEA_res_files)
            os.system(in_files['Rscript_path'] + 'Rscript ' + R_fold + 'ensemble_fisher.R ' + DEA_res_files)
            cal_mse_auc('ensemble_' + method, acq, platform, method, '_', len(cbn), save_res_fold,
                        dataset, cbn_str + '||' + wfs, in_files)
        elif method == 'hurdle':
            save_res_fold = save_fold + dataset + '/' + dataset + '_' + platform + '_' + method + '_' + cbn_str.replace('|', '-') + '.csv'
            if not os.path.exists(save_fold + dataset + '/'):
                os.makedirs(save_fold + dataset + '/')
            DEA_res_files = save_res_fold + ' ' + print_lab + ' ' + true_organism + ' ' + DE_organism + ' ' + true_logfc + ' ' + conss + ' ' + wfs
            print(in_files['Rscript_path'] + 'Rscript ' + R_fold + 'ensemble_hurdle.R ' + DEA_res_files)
            os.system(in_files['Rscript_path'] + 'Rscript ' + R_fold + 'ensemble_hurdle.R ' + DEA_res_files)
            cal_mse_auc('ensemble_' + method, acq, platform, method, '_', len(cbn), save_res_fold,
                        dataset, cbn_str + '||' + wfs, in_files)


def ensemble_top_k(platform, acq, method, res_fold, save_fold, DEA_res_fold, dataset, in_files, true_organism, DE_organism, K):

    ranking_all = pd.read_excel(res_fold + 'ranks_all_' + platform + '_' + acq + '.xlsx', sheet_name='ranking_all', header=0)
    # drop MSstats
    ranking_all = ranking_all[ranking_all['DEA'] != 'MSstats']
    ranking_all = ranking_all.sort_values(by=['avg_rank_mean'])
    ...
    for i in range(2, K):
        ...
        for j in range(i):
            top_wf_idx = j
            top_wfs.append(top_wf_idx)
            dea = ranking_all['DEA'].values[top_wf_idx]
            mtx = ranking_all['Matrix'].values[top_wf_idx]
            imput = ranking_all['Imput'].values[top_wf_idx]
            norm = ranking_all['normalization'].values[top_wf_idx]
            if imput == 'blank':
                imput = ''
            if norm == 'blank':
                norm = ''
        ...
        if method == 'set':
            for operation in ['min', 'max', 'median']:
                ...
                cal_mse_auc('ensemble_' + method + '_' + operation, acq, platform, method, operation, i, save_res_fold,
                            dataset, cbn_str + '||' + wfs, in_files)
        elif method == 'fisher':
            save_res_fold = save_fold + dataset + '/' + dataset + '_' + platform + '_' + method + '_' + cbn_str.replace('|', '-') + '.csv'
            if not os.path.exists(save_fold + dataset + '/'):
            ...
            print(in_files['Rscript_path'] + 'Rscript ' + R_fold + 'ensemble_fisher.R ' + DEA_res_files)
            os.system(in_files['Rscript_path'] + 'Rscript ' + R_fold + 'ensemble_fisher.R ' + DEA_res_files)
            cal_mse_auc('ensemble_' + method, acq, platform, method, '_', i, save_res_fold,
                        dataset, cbn_str + '||' + wfs, in_files)
        elif method == 'hurdle':
            save_res_fold = save_fold + dataset + '/' + dataset + '_' + platform + '_' + method + '_' + cbn_str.replace('|', '-') + '.csv'
            if not os.path.exists(save_fold + dataset + '/'):
...
if __name__ == '__main__':
    ...
    Rscript_pth = sys.argv[3]
    #user_save_fold = sys.argv[4]

    acq = 'TMT'

    for data_idx in range(0, 7):
        if platform == 'FragPipe':
            etys = ['mv', 'topk']
        else:
            etys = ['topk']
        for ensemble_type in etys:
            for method in ['set', 'fisher', 'hurdle']:

                res_fold = 'benchmark_res/ensemble/'
                save_fold = res_fold + acq + '/' + platform + '/' + ensemble_type + '/'

                root_fold = ''  # 'D:/data/benchmark/'
                R_fold = root_fold + 'codes/benchmark_R_scripts/'
                save_folder = res_fold + acq + '/' + platform + '/' + ensemble_type + '/'
                maxtrix_folder = root_fold + 'data/' + acq + '/' + platform + '/'
                Rscript_path = Rscript_pth  # 'D:/R-4.3.1/bin/'
                ...
                if platform == 'FragPipe':
                    ...
                    dataset_info_file = root_fold + 'data/dataset_info/' + acq + '_' + 'spn.txt'
                ...
                if platform == 'FragPipe':
                    main_output_protein_file = dataset_info['output_folder'][data_idx] + dataset_info['main_output_abundance_file'][
                    data_idx]
                    main_output_peptide_file = dataset_info['output_folder'][data_idx] + dataset_info['main_output_ratio_file'][
                    data_idx]
                elif platform == 'Maxquant':
                    main_output_protein_file = dataset_info['output_folder'][data_idx] + dataset_info['main_output_protein_file'][
                        data_idx]
                    main_output_peptide_file = dataset_info['output_folder'][data_idx] + dataset_info['main_output_peptide_file'][
                        data_idx]

                all_reported_protein_path = dataset_info['all_protein_file'][data_idx]

                msstats_design_file = dataset_info['msstat_design_path'][data_idx]
                if platform != 'FragPipe':
                    evidence_file = dataset_info['output_folder'][data_idx] + dataset_info['main_out_evidence_file'][
                        data_idx]
                    mstats_file = 'NULL'
                else:
                    evidence_file = 'NULL'
                    mstats_file = dataset_info['mstats_output'][data_idx]
                ...
                DEA_res_fold = 'benchmark_res/' + acq + '/' + platform + '/' + dataset + '/'
                if ensemble_type == 'mv':
                    ensemble_top_multi_view(platform, acq, method, res_fold, save_fold, DEA_res_fold, dataset, in_files,
                                            true_organism, DE_organism)
                elif ensemble_type == 'topk':
                    ensemble_top_k(platform, acq, method, res_fold, save_fold, DEA_res_fold, dataset, in_files, true_organism,
                                   DE_organism, K=20)
