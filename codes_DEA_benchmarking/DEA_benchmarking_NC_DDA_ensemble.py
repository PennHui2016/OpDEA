...
def run_all_group1(acq, tools, platform, imputs, normals, in_files, dataset, true_organism,
    ...
    for tool in tools:
        inten_types = ['top0','top3', 'LFQ', 'dlfq']  # ['LFQ']#, 'dlfq']#
...
def ensemble_top_k(platform, acq, method, res_fold, save_fold, DEA_res_fold, dataset, in_files, true_organism, DE_organism, K):
    ...
    ranking_all = pd.read_excel(res_fold + 'ranks_all_' + platform + '_' + acq + '.xlsx', sheet_name='ranking_all',
                                header=0)
    # drop MSstats
    ranking_all = ranking_all[ranking_all['DEA'] != 'MSstats']
    ...
    for i in range(2, K):
        ...
        if method == 'set':
            for operation in ['min', 'max', 'median']:
                save_res_fold = save_fold + dataset + '/' + dataset + '_' + platform + '_' + method + '_' + operation + '_top' + cbn_str + '.csv'
                if not os.path.exists(save_fold + dataset + '/'):
                    os.makedirs(save_fold + dataset + '/')
                DEA_res_files = save_res_fold + ' ' + operation + ' ' + print_lab + ' ' + true_organism + ' ' + DE_organism + ' ' + true_logfc + ' ' + conss + ' ' + wfs
                print('Rscript ' + R_fold + 'ensemble_set.R ' + DEA_res_files)
            ...
            save_res_fold = save_fold + dataset + '/' + dataset + '_' + platform + '_' + method + '_top' + cbn_str + '.csv'
            if not os.path.exists(save_fold + dataset + '/'):
                os.makedirs(save_fold + dataset + '/')
            DEA_res_files = save_res_fold + ' ' + print_lab + ' ' + true_organism + ' ' + DE_organism + ' ' + true_logfc + ' ' + conss + ' ' + wfs
            print('Rscript ' + R_fold + 'ensemble_fisher.R ' + DEA_res_files)
...
if __name__ == '__main__':
    ...
    for method in ['set', 'fisher', 'hurdle']:
        ...
        msstats_design_file = maxtrix_folder + dataset_info['msstat_design_path'][
            data_idx]
        if platform != 'FragPipe':
        ...
        if ensemble_type == 'mv':
            ensemble_top_multi_view(platform, acq, method, res_fold, save_fold, DEA_res_fold, dataset, in_files, true_organism, DE_organism)
        elif ensemble_type == 'topk':
            ensemble_top_k(platform, acq, method, res_fold, save_fold, DEA_res_fold, dataset, in_files, true_organism, DE_organism, K=20)
